...
public class AT_Elevation : OnlineMapsSinglePartElevationManager<OnlineMapsBingMapsElevationManager>
{
    /// <summary>
    /// Bing Maps API key
    /// </summary>
    ...
    public override void RequestNewElevationData()
    {
        ...
        int countX = TargetMap.width / s + 2;
        int countY = TargetMap.height / s + 2;

        double sx, sy, ex, ey;
        TargetMap.projection.TileToCoordinates(bufferPosition.x - 1, bufferPosition.y - 1, map.zoom, out sx, out sy);
        TargetMap.projection.TileToCoordinates(bufferPosition.x + countX + 1, bufferPosition.y + countY + 1, map.zoom, out ex, out ey);
